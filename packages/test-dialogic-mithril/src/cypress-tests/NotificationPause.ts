...
export default () => {
  const fns1 = createFns({ instance: notification, component: Default, className: "notification", title: "Default", timeout: 10000 });
  const fns2 = createFns({ instance: notification, component: Default, className: "notification", spawn: "2", title: "Spawn", timeout: 10000 });
  ...
  return {
    view: () => {
      return m(".test", [
        ...
        m(".control", { "data-test-id": "is-paused" }, `Is paused: ${notification.isPaused({ spawn: "2" })}`),
        m(".control", m(Remaining, { key: "NotificationPause", getRemaining: () => notification.getRemaining({ spawn: "2" }) })),
        m(".content", [
